...
-- Versão 5 - Adicionando campo de recomendações
...
-- Adicionar campo recommendations se não existir
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'problems' AND column_name = 'recommendations') THEN
        ALTER TABLE problems ADD COLUMN recommendations TEXT;
    END IF;
END $$;
